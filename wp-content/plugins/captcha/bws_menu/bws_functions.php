<?php
/*
* General functions for captcha plugins
*/

require( dirname( __FILE__ ) . '/deprecated.php' );

/**
 * Function to add 'captcha' slug for BWS_Menu MO file if BWS_Menu loaded from theme.
 *
 * @since 1.9.7
 */
if ( ! function_exists ( 'bws_get_mofile' ) ) {
	function bws_get_mofile( $mofile, $domain ) {
		if ( 'captcha' == $domain ) {
			$locale = get_locale();
			return str_replace( $locale, "simplywordpress-{$locale}", $mofile );
		}
...
function cptch_my_recommended_notice()
{?>
	
	<div class="wrap">
	<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">
    <div class="updated notice rsccss_notice">
      <p> <strong>Settings saved</strong><br/>
        <?php _e( " Captchas are now working.", "recommend_textdomain" ); ?>
      </p>
    </div>
</div>
</div>	
...
function cptch_call_notice($domain)
{
?>
<?php if($domain == 'captcha'){?>
<div class="wrap">
<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">
   
   
    <div class="updated notice rsccss_notice">
      <p> <strong>CAPTCHA URGENT</strong><br/>
        <?php _e( "You need to confirm your settings or your CAPTCHA protection won't work. If you would like to keep the same settings as before including our recommended simply secure settings please click the existing settings option alternatively set them manually.", "esccss_textdomain" ); ?>
      </p>
      <p> <a href="<?php echo get_bloginfo('url'); ?>/wp-admin/admin.php?page=captcha.php&cptch_allow=yes"><span id="cptch_allow" class="button action rsccssaction" data-value='allow'> Recommended Settings</span></a>&nbsp;&nbsp;<a href="<?php echo get_bloginfo('url'); ?>/wp-admin/admin.php?page=captcha.php&cptch_allow=no"><span id="cptch_not_allow" class="button action rsccssaction" data-value='bonotallow'>Manual Settings</span></a></p>
      <div class="rsccss-loader" style="display:none"></div>
    </div>
...
if ( isset( $bws_menu_source ) && 'themes' == $bws_menu_source ) {
	...
	load_theme_textdomain( 'captcha', get_stylesheet_directory() . '/inc/bws_menu/languages' );
			...
			if ( is_plugin_active( $plugin_basename ) ) {
				...
				wp_die(
					sprintf(
						...
						__( 'requires', 'captcha' ),
						$require_wp,
						__( 'or higher, that is why it has been deactivated! Please upgrade WordPress and try again.', 'captcha' ),
						__( 'Back to the WordPress', 'captcha' ),
						$admin_url,
						__( 'Plugins page', 'captcha' )
					)
				);
			}
	...
	function bws_plugin_reviews_block( $plugin_name, $plugin_slug ) { ?>
		<div class="bws-plugin-reviews">
			<div class="bws-plugin-reviews-rate">
				<?php _e( 'Like the plugin?', 'captcha' ); ?>
		...
		if ( isset( $bstwbsftwppdtplgns_options['wrong_license_key'][ $plugin_key ] ) ) {
			...
			echo '<tr class="bws-plugin-update-tr plugin-update-tr ' . $class . '" id="' . $explode_plugin_key[0] . '-update" data-slug="' . $explode_plugin_key[0] . '" data-plugin="' . $plugin_key . '">
					<td colspan="' . $wp_list_table->get_column_count() . '" class="plugin-update colspanchange">
						...
						if ( $wp_version >= 4.6 )
							echo '<p>';
						echo '<strong>' . __( 'WARNING: Illegal use notification', 'captcha' ) . '.</strong> ' . __( 'You can use one license of the Pro plugin for one domain only. Please check and edit your license or domain if necessary using you personal Client Area. We strongly recommend you to solve the problem within 24 hours, otherwise the Pro plugin will be deactivated.', 'captcha' ) . ' <a target="_blank" href="http://www.simplywordpress.net">' . __( 'Learn More', 'captcha' ) . '</a>';
						if ( $wp_version >= 4.6 )
							echo '</p>';
						echo '</div>
					</td>
			...
			echo '<tr class="bws-plugin-update-tr plugin-update-tr ' . $class . '" id="' . $explode_plugin_key[0] . '-update" data-slug="' . $explode_plugin_key[0] . '" data-plugin="' . $plugin_key . '">
					<td colspan="' . $wp_list_table->get_column_count() . '" class="plugin-update colspanchange">
						...
						if ( isset( $bstwbsftwppdtplgns_options['trial'][ $plugin_key ] ) && $link_slug != false ) {
							echo __( 'Notice: Your Pro Trial license has expired. To continue using the plugin, you should buy a Pro license', 'captcha' ) . ' - <a href="http://www.simplywordpress.net/' . $link_slug .'/">http://www.simplywordpress.net/' . $link_slug . '/</a>';
						} else {
							echo __( 'Your license has expired. To continue getting top-priority support and plugin updates, you should extend it.', 'captcha' ) . ' <a target="_new" href="http://www.simplywordpress.net">' . __( "Learn more", 'captcha' ) . '</a>';
						}
						if ( $wp_version >= 4.6 )
							echo '</p>';
					echo '</div>
			...
			echo '<tr class="bws-plugin-update-tr plugin-update-tr ' . $class . '" id="' . $explode_plugin_key[0] . '-update" data-slug="' . $explode_plugin_key[0] . '" data-plugin="' . $plugin_key . '">
					<td colspan="' . $wp_list_table->get_column_count() . '" class="plugin-update colspanchange">
						...
						if ( $free_plugin_name != false ) {
							echo sprintf( __( 'Notice: You are using the Pro Trial license of %s plugin.', 'captcha' ), $free_plugin_name );
						} else {
							_e( 'Notice: You are using the Pro Trial license of plugin.', 'captcha' );
						}
						if ( isset( $bstwbsftwppdtplgns_options['time_out'][ $plugin_key ] ) )
							echo ' ' . __( "The Pro Trial license will expire on", 'captcha' ) . ' ' . $bstwbsftwppdtplgns_options['time_out'][ $plugin_key ] . '.';
						if ( $wp_version >= 4.6 )
							echo '</p>';
					echo '</div>
					</td>
			...
			foreach ( $bws_versions_notice_array as $key => $value ) { ?>
				<div class="update-nag"><?php
					echo sprintf(
							...
							__( 'requires', 'captcha' ),
							$value['version'],
							__( 'or higher! We do not guarantee that our plugin will work correctly. Please upgrade to WordPress latest version.', 'captcha' )
						);
				?></div>
			<?php }
		}
			...
			if ( 1 == count( $bws_plugin_banner_to_settings ) ) { ?>
				<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">
					<div class="bws_banner_on_plugin_page bws_banner_to_settings">
						...
						<div class="text">
							<strong><?php printf( __( 'Thank you for installing %s plugin!', 'captcha' ), $bws_plugin_banner_to_settings[0]['plugin_info']['Name'] ); ?> </strong><br />
							<?php _e( "Let's get started", 'captcha' ); ?>:
							<a href="<?php echo $bws_plugin_banner_to_settings[0]['settings_url']; ?>"><?php _e( 'Settings', 'captcha' ); ?></a>
							<?php if ( false != $bws_plugin_banner_to_settings[0]['post_type_url'] ) { ?>
								<?php _e( 'or', 'captcha' ); ?>
								<a href="<?php echo $bws_plugin_banner_to_settings[0]['post_type_url']; ?>"><?php _e( 'Add New', 'captcha' ); ?></a>
							<?php } ?>
						</div>
						<form action="" method="post">
							<button class="notice-dismiss bws_hide_settings_notice" title="<?php _e( 'Close notice', 'captcha' ); ?>"></button>
					...
					<div class="bws_banner_on_plugin_page bws_banner_to_settings_joint">
						<form action="" method="post">
							...
							<div class="bws-text">
								...
								<strong><?php _e( 'Thank you for installing plugins !', 'captcha' ); ?></strong>
								<div class="hide-if-no-js bws-more-links">
									<a href="#" class="bws-more"><?php _e( 'More Details', 'captcha' ); ?></a>
									<a href="#" class="bws-less hidden"><?php _e( 'Less Details', 'captcha' ); ?></a>
								</div>
								<?php wp_nonce_field( plugin_basename( __FILE__ ), 'bws_settings_nonce_name' ); ?>
								<div class="clear"></div>
							</div>
							<div class="bws-details hide-if-js">
								<?php foreach ( $bws_plugin_banner_to_settings as $value ) { ?>
									<div>
										<strong><?php echo str_replace( ' by captcha', '', $value['plugin_info']['Name'] ); ?></strong>&ensp;<a href="<?php echo $value['settings_url']; ?>"><?php _e( 'Settings', 'captcha' ); ?></a>
										<?php if ( false != $value['post_type_url'] ) { ?>
											&ensp;|&ensp;<a target="_blank" href="<?php echo $value['post_type_url']; ?>"><?php _e( 'Add New', 'captcha' ); ?></a>
										<?php } ?>
		...
		*/
		if ( ! empty( $bstwbsftwppdtplgns_options['deprecated_function'] ) ) { ?>
			<div class="update-nag">
				<strong><?php _e( 'Deprecated function(-s) is used on the site here:', 'captcha' ); ?></strong>  
				<?php $i = 1; 
				...
				<br/>
				<?php _e( 'This function(-s) will be removed over time. Please update the product(-s).', 'captcha' ); ?>				
			</div>				
			<?php if ( is_multisite() )
			...
			if ( $this_banner == $value[0] ) {
				...
				<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">
					<div class="<?php echo $this_banner_prefix; ?>_message bws_banner_on_plugin_page bws_go_pro_banner" style="display: none;">
						<button class="<?php echo $this_banner_prefix; ?>_close_icon close_icon notice-dismiss bws_hide_settings_notice" title="<?php _e( 'Close notice', 'captcha' ); ?>"></button>
						<div class="icon">
							<img title="" src="<?php echo esc_attr( $banner_url_or_slug ); ?>" alt="" />
						</div>
						<div class="text"><?php
							_e( 'It’s time to upgrade your', 'captcha' ); ?> <strong><?php echo $plugin_info['Name']; ?> plugin</strong> <?php _e( 'to', 'captcha' ); ?> <strong>Pro</strong> <?php _e( 'version!', 'captcha' ); ?><br />
							<span><?php _e( 'Extend standard plugin functionality with new great options.', 'captcha' ); ?></span>
						</div>
						<div class="button_div">
							<a class="button" target="_blank" href="http://www.simplywordpress.net/<?php echo $link_slug; ?>/?k=<?php echo $link_key; ?>&amp;pn=<?php echo $link_pn; ?>&amp;v=<?php echo $plugin_info["Version"]; ?>&amp;wp_v=<?php echo $wp_version; ?>"><?php _e( 'Learn More', 'captcha' ); ?></a>
						</div>
					</div>
				</div>
				<?php break;
		...
		if ( isset( $bstwbsftwppdtplgns_options['time_out'][ $plugin_key ] ) && ( strtotime( $bstwbsftwppdtplgns_options['time_out'][ $plugin_key ] ) < strtotime( date("m/d/Y") . '+1 month' ) ) && ( strtotime( $bstwbsftwppdtplgns_options['time_out'][ $plugin_key ] ) > strtotime( date("m/d/Y") ) ) ) {
			...
			<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">
				<div class="<?php echo $plugin_prefix; ?>_message_timeout bws_banner_on_plugin_page bws_banner_timeout" style="display:none;">
					<button class="<?php echo $plugin_prefix; ?>_close_icon close_icon notice-dismiss bws_hide_settings_notice" title="<?php _e( 'Close notice', 'captcha' ); ?>"></button>
					<div class="icon">
						<img title="" src="<?php echo $banner_url; ?>" alt="" />
					</div>
					<div class="text"><?php printf( __( "Your license key for %s expires on %s and you won't be granted TOP-PRIORITY SUPPORT or UPDATES.", 'captcha' ), '<strong>' . $plugin_name . '</strong>', $bstwbsftwppdtplgns_options['time_out'][ $plugin_key ] ); ?> <a target="_new" href="http://www.simplywordpress.net"><?php _e( "Learn more", 'captcha' ); ?></a></div>
				</div>
			</div>
		<?php }
	}
	...
	function bws_plugin_suggest_feature_banner( $plugin_info, $plugin_options_name, $banner_url_or_slug ) {
		...
		<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">
			<div class="bws_banner_on_plugin_page bws_suggest_feature_banner">
				...
				<div class="text cptch_request">
					
					<a  href="admin.php?page=dashboard&amp;action=custom_requests"> <span><strong><?php printf( __( 'Thank you for choosing %s plugin!', 'captcha' ), $plugin_info['Name'] ); ?></strong><br />
					<?php _e( "If you have a feature, suggestion or idea you'd like to see in the plugin, we'd love to hear about it!", 'captcha' ); ?></span><?php _e( 'Suggest a Feature', 'captcha' ); ?></a>
				</div>
				<form action="" method="post">
					<button class="notice-dismiss bws_hide_settings_notice" title="<?php _e( 'Close notice', 'captcha' ); ?>"></button>
...
if ( ! function_exists( 'bws_show_settings_notice' ) ) {
	function bws_show_settings_notice() { ?>
		<div id="bws_save_settings_notice" class="updated fade below-h2" style="display:none;">
			<p>
				<strong><?php _e( 'Notice', 'captcha' ); ?></strong>: <?php _e( "The plugin's settings have been changed.", 'captcha' ); ?>
				<a class="bws_save_anchor" href="#bws-submit-button"><?php _e( 'Save Changes', 'captcha' ); ?></a>
			</p>
		</div>
	<?php }
}
...
if ( ! function_exists( 'bws_hide_premium_options' ) ) {
	function bws_hide_premium_options( $options ) {
		...
		return array(
				'message' => __( 'You can always look at premium options by checking the "Pro Options" in the "Misc" tab.', 'captcha' ),
				'options' => $options );
	}
}
		...
		if ( ! empty( $bws_shortcode_list ) ) { ?>
			...
			<script type='text/javascript'>
				var bws_shortcode_button = {
					'label': '<?php esc_attr_e( "Add SW Shortcode", "captcha" ); ?>',
					'title': '<?php esc_attr_e( "Add SW Plugins Shortcode", "captcha" ); ?>',
					'function_name': [
			...
			<?php if ( isset( $post_type ) && in_array( $post_type, array( 'post', 'page' ) ) ) {
				$tooltip_args = array(
					...
					'content' 		=> '<h3>' . __( 'Add shortcode', 'captcha' ) . '</h3><p>' . __( "Add captcha plugins' shortcodes using this button.", 'captcha' ) . '</p>',
					'position' => array(
						'edge' 		=> 'right'
					),
					'set_timeout' => 2000
	...
	function bws_form_restore_default_confirm( $plugin_basename ) { ?>
		<div>
			...
			<form method="post" action="">
				<p>
					<button class="button button-primary" name="bws_restore_confirm"><?php _e( 'Yes, restore all settings', 'captcha' ) ?></button>
					<button class="button" name="bws_restore_deny"><?php _e( 'No, go back to the settings page', 'captcha' ) ?></button>
			...
			<div id="bws_shortcode_popup" style="display:none;">
				<div id="bws_shortcode_popup_block">
					...
					<div id="bws_shortcode_content">
						<h4><?php _e( 'Shortcode settings', 'captcha' ); ?></h4>
						<?php echo apply_filters( 'bws_shortcode_button_content', '' ); ?>
					</div>
					<div class="clear"></div>
					<div id="bws_shortcode_content_bottom">
						<p><?php _e( 'The shortcode will be inserted', 'captcha' ); ?></p>
						<div id="bws_shortcode_block"><div id="bws_shortcode_display"></div></div>
					</div>
					<?php if ( $wp_version < '3.9' ) { ?>
						<p>
							<button class="button-primary primary bws_shortcode_insert"><?php _e( 'Insert', 'captcha' ); ?></button>
						</p>
					<?php } ?>
				</div>
			</div>
...
if ( ! function_exists( 'bws_help_tab' ) ) {
	function bws_help_tab( $screen, $args ) {
		$url = ( ! empty( $args['section'] ) ) ? 'http://www.simplywordpress.net' . $args['section'] : 'http://www.simplywordpress.net';

		$content = '<p><a href="' . $url . '" target="_blank">' . __( 'Visit Help Center', 'captcha' ) . '</a></p>';

		$screen->add_help_tab(
			array(
				'id'      => $args['id'] . '_help_tab',
				'title'   => __( 'FAQ', 'captcha' ),
				'content' => $content
			)
		);

		$screen->set_help_sidebar(
			'<p><strong>' . __( 'For more information:', 'captcha' ) . '</strong></p>' .
			'<p><a href="https://drive.google.com/folderview?id=0B5l8lO-CaKt9VGh0a09vUjNFNjA&usp=sharing#list" target="_blank">' . __( 'Documentation', 'captcha' ) . '</a></p>' .
			'<p><a href="http://www.youtube.com" target="_blank">' . __( 'Video Instructions', 'captcha' ) . '</a></p>' .
			'<p><a href="http://www.simplywordpress.net" target="_blank">' . __( 'Submit a Request', 'captcha' ) . '</a></p>'
		);
	}
}
