<?php
/*
* Deprecated functions for simplywordpress plugins
*/
...
if ( ! function_exists ( 'bws_general_menu' ) ) {
	function bws_general_menu() {
		...
		if ( ! $bws_general_menu_exist ) {
			...
			add_submenu_page( 'bws_panel', __( 'Plugins', 'simplywordpress' ), __( 'Plugins', 'simplywordpress' ), 'manage_options', 'bws_panel', 'bws_add_menu_render' );
			add_submenu_page( 'bws_panel', __( 'Themes', 'simplywordpress' ), __( 'Themes', 'simplywordpress' ), 'manage_options', 'bws_themes', 'bws_add_menu_render' );
			add_submenu_page( 'bws_panel', __( 'System Status', 'simplywordpress' ), __( 'System Status', 'simplywordpress' ), 'manage_options', 'bws_system_status', 'bws_add_menu_render' );
			...
			if ( '' != $license_key ) {
				...
				if ( is_array( $plugins_all ) && !empty( $plugins_all ) && isset( $current ) && is_array( $current->response ) ) {
					...
					if ( is_wp_error( $raw_response ) || 200 != wp_remote_retrieve_response_code( $raw_response ) ) {
						$result['error'] = __( 'Something went wrong. Please try again later. If the error appears again, please contact us', 'simplywordpress' ) . ' <a href=https://www.simplywordpress.net>simplywordpress</a>. ' . __( 'We are sorry for inconvenience.', 'simplywordpress' );
					} else {
						$response = maybe_unserialize( wp_remote_retrieve_body( $raw_response ) );
						if ( is_array( $response ) && !empty( $response ) ) {
							foreach ( $response as $key => $value ) {
								if ( "wrong_license_key" == $value->package ) {
									$result['error'] = __( 'Wrong license key.', 'simplywordpress' );
								} else if ( "wrong_domain" == $value->package ) {
									$result['error'] = __( 'This license key is bound to another site.', 'simplywordpress' );
								} else if ( "time_out" == $value->package ) {
									$result['message'] = __( 'This license key is valid, but Your license has expired. If you want to update our plugin in future, you should extend the license.', 'simplywordpress' );
								} elseif ( "you_are_banned" == $value->package ) {
									$result['error'] = __( "Unfortunately, you have exceeded the number of available tries.", 'simplywordpress' );
								} elseif ( "duplicate_domen_for_trial" == $value->package ) {
									$result['error'] = __( "Unfortunately, the Pro Trial licence was already installed to this domain. The Pro Trial license can be installed only once.", 'simplywordpress' );
								}
								if ( empty( $result['message'] ) && empty( $result['error'] ) ) {
									if ( isset( $value->trial ) )
										$result['message'] = __( 'The Pro Trial license key is valid.', 'simplywordpress' );
									else
										$result['message'] = __( 'The license key is valid.', 'simplywordpress' );

									if ( ! empty( $value->time_out ) )
										$result['message'] .= ' ' . __( 'Your license will expire on', 'simplywordpress' ) . ' ' . $value->time_out . '.';

									if ( isset( $value->trial ) && $trial_plugin != false )
										$result['message'] .= ' ' . sprintf( __( 'In order to continue using the plugin it is necessary to buy a %s license.', 'simplywordpress' ), '<a href="https://www.simplywordpress.net/' . $trial_plugin['link_slug'] . '/?k=' . $trial_plugin['link_key'] . '&pn=' . $trial_plugin['link_pn'] . '&v=' . $trial_plugin['plugin_info']['Version'] . '&wp_v=' . $wp_version . '" target="_blank" title="' . $trial_plugin['plugin_info']['Name'] . '">Pro</a>' );
				...
				$result['error'] = __( 'Please, enter your license key', 'simplywordpress' );
			}
		}
		return $result;
	}
...
if ( ! function_exists ( 'bws_check_pro_license_form' ) ) {
	function bws_check_pro_license_form( $plugin_basename ) {
		...
		<form method="post" action="">
			<p><?php echo _e( 'If necessary, you can check if the license key is correct or reenter it in the field below. You can find your license key on your personal page - Client Area - on our website', 'simplywordpress' ) . ' <a href="https://www.simplywordpress.net">www.simplywordpress.net</a> ' . __( '(your username is the email address specified during the purchase). If necessary, please submit "Lost your password?" request.', 'simplywordpress' ); ?></p>
			<p>
				<input type="text" maxlength="100" name="bws_license_key" value="<?php echo $license_key; ?>" />
				<input type="hidden" name="bws_license_submit" value="submit" />
				<input type="submit" class="button" value="<?php _e( 'Check license key', 'simplywordpress' ) ?>" />
...
if ( ! function_exists( 'bws_go_pro_from_trial_tab' ) ) {
	function bws_go_pro_from_trial_tab( $plugin_info, $plugin_basename, $page, $link_slug, $link_key, $link_pn, $trial_license_is_set = true ) {
		...
		if ( $trial_license_is_set ) { ?>
			<form method="post" action="">
				<p>
					<?php echo sprintf( __( 'In order to continue using the plugin it is necessary to buy a %s license.', 'simplywordpress' ), '<a href="www.simplywordpress.net/' . $link_slug . '/?k=' . $link_key . '&amp;pn=' . $link_pn . '&amp;v=' . $plugin_info["Version"] . '&amp;wp_v=' . $wp_version .'" target="_blank" title="' . $plugin_info["Name"] . '">Pro</a>' ) . ' ';
					_e( 'After that, you can activate it by entering your license key.', 'simplywordpress' ); ?><br />
					<span class="bws_info">
						<?php _e( 'License key can be found in the', 'simplywordpress' ); ?>
						<a href="https://www.simplywordpress.net">Client Area</a>
						<?php _e( '(your username is the email address specified during the purchase).', 'simplywordpress' ); ?>
					</span>
				</p>
				<?php if ( isset( $bstwbsftwppdtplgns_options['go_pro'][ $plugin_basename ]['count'] ) &&
					...
					<p>
						...
						<input disabled="disabled" type="submit" class="button-primary" value="<?php _e( 'Activate', 'simplywordpress' ); ?>" />
					</p>
					<p><?php _e( "Unfortunately, you have exceeded the number of available tries per day.", 'simplywordpress' ); ?></p>
				<?php } else { ?>
					<p>
						...
						<input type="hidden" name="bws_license_submit" value="submit" />
						<input type="submit" class="button-primary" value="<?php _e( 'Activate', 'simplywordpress' ); ?>" />
						<?php wp_nonce_field( $plugin_basename, 'bws_license_nonce_name' ); ?>
					</p>
				<?php } ?>
			</form>
		<?php } else { ?>
			...
			<p><?php _e( "Congratulations! The Pro license of the plugin is activated successfully.", 'simplywordpress' ); ?></p>
			<p>
				<?php _e( "Please, go to", 'simplywordpress' ); ?> <a href="admin.php?page=<?php echo $page; ?>"><?php _e( 'the setting page', 'simplywordpress' ); ?></a>
				(<?php _e( "You will be redirected automatically in 5 seconds.", 'simplywordpress' ); ?>)
			</p>
		<?php }
	}
}
	...
	function bws_go_pro_tab_check( $plugin_basename, $plugin_options_name = false, $is_network_option = false ) {
		...
		if ( ! empty( $plugin_options_name ) && isset( $_POST['bws_hide_premium_options_submit'] ) && check_admin_referer( $plugin_basename, 'bws_license_nonce_name' ) ) {
			...
			if ( !empty( $plugin_options['hide_premium_options'] ) ) {
				...
				$result['message'] = __( 'Check premium options on the plugin settings page!', 'simplywordpress' );
			}
		}
								...
								if ( is_array( $response ) && !empty( $response ) ) {
									foreach ( $response as $key => $value ) {
										if ( "wrong_license_key" == $value->package ) {
											...
											$result['error'] = __( "Unfortunately, you have exceeded the number of available tries per day. Please, upload the plugin manually.", 'simplywordpress' );
										} elseif ( "time_out" == $value->package ) {
											$result['error'] = sprintf( __( "Unfortunately, Your license has expired. To continue getting top-priority support and plugin updates, you should extend it in your %s", 'simplywordpress' ), ' <a href="https://www.simplywordpress.net">Client Area</a>' );
										} elseif ( "duplicate_domen_for_trial" == $value->package ) {
											$result['error'] = __( "Unfortunately, the Pro licence was already installed to this domain. The Pro Trial license can be installed only once.", 'simplywordpress' );
										}
									}
									if ( empty( $result['error'] ) ) {
										...
										if ( ! $received_content ) {
											$result['error'] = __( "Failed to download the zip archive. Please, upload the plugin manually", 'simplywordpress' );
										} else {
											if ( is_writable( $uploadDir["path"] ) ) {
												...
												if ( file_put_contents( $file_put_contents, $received_content ) ) {
													...
													if ( class_exists( 'ZipArchive' ) ) {
														...
														if ( $zip->open( $file_put_contents ) === TRUE ) {
															...
															$result['error'] = __( "Failed to open the zip archive. Please, upload the plugin manually", 'simplywordpress' );
														}
													} elseif ( class_exists( 'Phar' ) ) {
														$phar = new PharData( $file_put_contents );
														$phar->extractTo( WP_PLUGIN_DIR );
													} else {
														$result['error'] = __( "Your server does not support either ZipArchive or Phar. Please, upload the plugin manually", 'simplywordpress' );
													}
												...
												$result['error'] = __( "UploadDir is not writable. Please, upload the plugin manually", 'simplywordpress' );
											}
										}
									...
									$result['error'] = __( "Something went wrong. Try again later or upload the plugin manually. We are sorry for inconvenience.", 'simplywordpress' );
								}
							}
						}
					} else {
...
if ( ! function_exists ( 'bws_form_restore_default_settings' ) ) {
	function bws_form_restore_default_settings( $plugin_basename, $change_permission_attr = '' ) { ?>
		<form method="post" action="">
			<p><?php _e( 'Restore all plugin settings to defaults', 'simplywordpress' ); ?></p>
			<p>
				<input <?php echo $change_permission_attr; ?> type="submit" class="button" value="<?php _e( 'Restore settings', 'simplywordpress' ); ?>" />
			</p>
...
if ( ! function_exists( 'bws_custom_code_tab' ) ) {
	function bws_custom_code_tab() {
		if ( ! current_user_can( 'edit_plugins' ) )
			wp_die( __( 'You do not have sufficient permissions to edit plugins for this site.', 'simplywordpress' ) );
		...
		if ( isset( $_REQUEST['bws_update_custom_code'] ) && check_admin_referer( 'bws_update_' . $css_file ) ) {
			...
			if ( $f = fopen( $real_css_file, 'w+' ) ) {
				...
				fclose( $f );
				$message .= sprintf( __( 'File %s edited successfully.', 'simplywordpress' ), '<i>' . $css_file . '</i>' ) . ' ';
			} else {
				$error .= __( 'Not enough permissions to create or update the file', 'simplywordpress' ) . ' ' . $real_css_file . '. ';
			}
			...
			if ( file_exists( $index_file ) ) {
				...
				if ( $f = fopen( $real_php_file, 'w+' ) ) {
					...
					fclose( $f );
					$message .= sprintf( __( 'File %s edited successfully.', 'simplywordpress' ), '<i>' . $php_file . '</i>' );
				} else {
					$error .= __( 'Not enough permissions to create or update the file', 'simplywordpress' ) . ' ' . $real_php_file . '. ';
				}
			} else {
				$error .= __( 'Not enough permissions to create the file', 'simplywordpress' ) . ' ' . $index_file . '. ';
			}

			if ( ! empty( $error ) )
				$error .= ' <a href="https://codex.wordpress.org/Changing_File_Permissions" target="_blank">' . __( 'Learn more', 'simplywordpress' ) . '</a>';
		...
		<form action="" method="post">
			<?php foreach ( array( 'css', 'php' ) as $extension ) { ?>
				<p>
					<?php if ( 'css' == $extension )
						_e( 'These styles will be added to the header on all pages of your site.', 'simplywordpress' );
					else
						printf( __( 'This PHP code will be hooked to the %s action and will be printed on front end only.', 'simplywordpress' ), '<a href="http://codex.wordpress.org/Plugin_API/Action_Reference/init" target="_blank"><code>init</code></a>' ); ?>
				</p>
				<p><big>
					<?php if ( ! file_exists( ${"real_{$extension}_file"} ) || ( is_writeable( ${"real_{$extension}_file"} ) ) ) {
						echo __( 'Editing', 'simplywordpress' ) . ' <strong>' . ${"{$extension}_file"} . '</strong>';
					} else {
						echo __( 'Browsing', 'simplywordpress' ) . ' <strong>' . ${"{$extension}_file"} . '</strong>';
					} ?>
				</big></p>
				<p><label><input type="checkbox" name="bws_custom_<?php echo $extension; ?>_active" value="1" <?php if ( ${"is_{$extension}_active"} ) echo "checked"; ?> />	<?php _e( 'Activate', 'simplywordpress' ); ?></label></p>
				...
				<p class="description">
					<a href="<?php echo ( 'css' == $extension ) ? 'https://developer.mozilla.org/en-US/docs/Web/Guide/CSS/Getting_started' : 'http://php.net/' ?>" target="_blank">
						<?php printf( __( 'Learn more about %s', 'simplywordpress' ), strtoupper( $extension ) ); ?>
					</a>
				</p>
			<?php }
			if ( ( ! file_exists( $real_css_file ) || is_writeable( $real_css_file ) ) && ( ! file_exists( $real_php_file ) || is_writeable( $real_php_file ) ) ) { ?>
				<p class="submit">
					...
					<?php submit_button( __( 'Save Changes', 'simplywordpress' ), 'primary', 'submit', false );
...
if ( ! function_exists( 'bws_go_pro_tab_show' ) ) {
	function bws_go_pro_tab_show( $bws_hide_premium_options_check, $plugin_info, $plugin_basename, $page, $pro_page, $bws_license_plugin, $link_slug, $link_key, $link_pn, $pro_plugin_is_activated = false, $trial_days_number = false ) {
		...
		if ( $pro_plugin_is_activated ) { ?>
			...
			<p><?php _e( "Congratulations! Pro version of the plugin is  installed and activated successfully.", 'simplywordpress' ); ?></p>
			<p>
				<?php _e( "Please, go to", 'simplywordpress' ); ?> <a href="admin.php?page=<?php echo $pro_page; ?>"><?php _e( 'the setting page', 'simplywordpress' ); ?></a>
				(<?php _e( "You will be redirected automatically in 5 seconds.", 'simplywordpress' ); ?>)
			</p>
		<?php } else {
			if ( $bws_hide_premium_options_check ) { ?>
				<form method="post" action="">
					<p>
						...
						<input type="submit" class="button" value="<?php _e( 'Show Pro features', 'simplywordpress' ); ?>" />
						<?php wp_nonce_field( $plugin_basename, 'bws_license_nonce_name' ); ?>
					</p>
				</form>
			<?php } ?>
			<form method="post" action="">
				<p>
					<?php _e( 'Enter your license key to install and activate', 'simplywordpress' ); ?>
					<a href="https://www.simplywordpress.net/<?php echo $link_slug; ?>/?k=<?php echo $link_key; ?>&amp;pn=<?php echo $link_pn; ?>&amp;v=<?php echo $plugin_info["Version"]; ?>&amp;wp_v=<?php echo $wp_version; ?>" target="_blank" title="<?php echo $plugin_info["Name"]; ?> Pro">Pro</a>
					<?php _e( 'version of the plugin.', 'simplywordpress' ); ?><br />
					<span class="bws_info">
						<?php _e( 'License key can be found in the', 'simplywordpress' ); ?>
						<a href="https://www.simplywordpress.net">Client Area</a>
						<?php _e( '(your username is the email address specified during the purchase).', 'simplywordpress' ); ?>
				...
				<?php if ( $trial_days_number !== false )
					$trial_days_number = __( 'or', 'simplywordpress' ) . ' <a href="https://www.simplywordpress.net/' . $link_slug . '/trial/" target="_blank">' . sprintf( __( 'Start Your Free %s-Day Trial Now', 'simplywordpress' ), $trial_days_number ) . '</a>';
