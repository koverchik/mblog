...
if ( ! class_exists( 'Bws_Settings_Tabs' ) ) {
	class Bws_Settings_Tabs {
		...
		public function __construct( $args = array() ) {
			...
			$args['plugins_info']['Name'] = str_replace( ' by simplywordpress', '', $args['plugins_info']['Name'] );
					...
					<div id="poststuff">
						<div id="post-body" class="metabox-holder columns-2">
							...
							<div id="postbox-container-1" class="postbox-container">
								<div class="meta-box-sortables ui-sortable">
									<div id="submitdiv" class="postbox">
										<h3 class="hndle"><?php _e( 'Information', 'simplywordpress' ); ?></h3>
										<div class="inside">
											<div class="submitbox" id="submitpost">
													...
													<div id="misc-publishing-actions">
														<?php if ( $this->is_pro ) {
															if ( ! isset( $bstwbsftwppdtplgns_options['time_out'][ $this->plugin_basename ] ) || isset( $bstwbsftwppdtplgns_options['wrong_license_key'][ $this->plugin_basename ] ) ) {
																$license_type = 'Pro';
																$license_status = __( 'Inactive', 'simplywordpress' ) . ' <a href="#' . $this->prefix . '_license_tab" class="bws_trigger_tab_click">' . __( 'Learn More', 'simplywordpress' ) . '</a>';
															} else {
																...
																if ( isset( $bstwbsftwppdtplgns_options['trial'][ $this->plugin_basename ] ) ) { 
																	...
																	if ( $finish < $today ) {
																		$license_status = __( 'Expired', 'simplywordpress' );
																	} else {
																		$daysleft = floor( ( $finish - $today ) / ( 60*60*24 ) );
																		$license_status = sprintf( __( '%s day(-s) left', 'simplywordpress' ), $daysleft );
																	}
																	$license_status .= '. <a target="_blank" href="' . $this->plugins_info['PluginURI'] . '">' . __( 'Upgrade to Pro', 'simplywordpress' ) . '</a>';							 		
																} else {
																	$license_type = isset( $bstwbsftwppdtplgns_options['nonprofit'][ $this->plugin_basename ] ) ? 'Nonprofit Pro' : 'Pro';
																	if ( $finish < $today ) {
																		$license_status = sprintf( __( 'Expired on %s', 'simplywordpress' ), $bstwbsftwppdtplgns_options['time_out'][ $this->plugin_basename ] ) . '. <a target="_blank" href="https://www.simplywordpress.net">' . __( 'Renew Now', 'simplywordpress' ) . '</a>';
																	} else {
																		$license_status = __( 'Active', 'simplywordpress' );
																	}
																}
															} ?>
															<div class="misc-pub-section">
																<strong><?php _e( 'License', 'simplywordpress' ); ?>:</strong> <?php echo $license_type; ?>
															</div>
															<div class="misc-pub-section">
																<strong><?php _e( 'Status', 'simplywordpress' ); ?>:</strong> <?php echo $license_status; ?>
															</div><!-- .misc-pub-section -->
														<?php } ?>
														<div class="misc-pub-section">
															<strong><?php _e( 'Version', 'simplywordpress' ); ?>:</strong> <?php echo $this->plugins_info['Version']; ?>
														</div><!-- .misc-pub-section -->
													</div>
													<div class="clear"></div>
												</div>
												<div id="major-publishing-actions">
													<div id="publishing-action">
														...
														<input id="bws-submit-button" type="submit" class="button button-primary button-large" value="<?php _e( 'Save Changes', 'simplywordpress' ); ?>" />
							...
							<div id="postbox-container-2" class="postbox-container">
								...
								<div class="submit">
									<input type="submit" class="button button-primary button-large" value="<?php _e( 'Save Changes', 'simplywordpress' ); ?>" />
								</div>								
		...
		public function save_all_tabs_options() {
			...
			if ( isset( $_POST['bws_restore_confirm'] ) && check_admin_referer( $this->plugin_basename, 'bws_settings_nonce_name' ) ) {
				...
				$message = __( 'All plugin settings were restored.', 'simplywordpress' );
		...
		private function tab_custom_code() { ?>
			<h3 class="bws_tab_label"><?php _e( 'Custom Code', 'simplywordpress' ); ?></h3>
			<?php $this->help_phrase(); ?>
			<hr>
			<?php if ( ! current_user_can( 'edit_plugins' ) ) {
				echo '<p>' . __( 'You do not have sufficient permissions to edit plugins for this site.', 'simplywordpress' ) . '</p>';
				return;
			}

			$list = array( 
				'css' => array( 'description' 	=> __( 'These styles will be added to the header on all pages of your site.', 'simplywordpress' ),
							'learn_more_link'	=> 'https://developer.mozilla.org/en-US/docs/Web/Guide/CSS/Getting_started' 
						), 
				'php' => array( 'description' 	=> sprintf( __( 'This PHP code will be hooked to the %s action and will be printed on front end only.', 'simplywordpress' ), '<a href="http://codex.wordpress.org/Plugin_API/Action_Reference/init" target="_blank"><code>init</code></a>' ),
							'learn_more_link'	=> 'http://php.net/' 
						),  
				'js' => array( 'description' 	=> __( 'These code will be added to the header on all pages of your site.', 'simplywordpress' ),
			...
			if ( ! $this->custom_code_args['css_writeable'] ||
				...
				<p><em><?php printf( __( 'You need to make this files writable before you can save your changes. See %s the Codex %s for more information.', 'simplywordpress' ),
			...
			foreach ( $list as $extension => $extension_data ) {
				...
				<p><big>
					...
					<?php if ( ! $this->custom_code_args["{$extension}_writeable"] )
						echo '(' . __( 'Browsing', 'simplywordpress' ) . ')'; ?>
				</big></p>
				<p class="bws_info">
					...
					<?php printf( __( 'Activate custom %s code.', 'simplywordpress' ), $name ); ?>
				</p>
				...
				<p class="bws_info">
					...
					<a href="<?php echo $extension_data['learn_more_link']; ?>" target="_blank">
						<?php printf( __( 'Learn more about %s', 'simplywordpress' ), $name ); ?>
					</a>
				</p>
			<?php }
		}
		...
		private function tab_misc() { 
			global $bstwbsftwppdtplgns_options; ?>
			<h3 class="bws_tab_label"><?php _e( 'Miscellaneous Settings', 'simplywordpress' ); ?></h3>
			<?php $this->help_phrase(); ?>
			<hr>
			<?php /**
			...
			if ( ! $this->forbid_view && ! empty( $this->change_permission_attr ) ) { ?>
				<div class="error inline bws_visible"><p><strong><?php _e( "Notice", 'simplywordpress' ); ?>:</strong> <strong><?php printf( __( "It is prohibited to change %s settings on this site in the %s network settings.", 'simplywordpress' ), $this->plugins_info["Name"], $this->plugins_info["Name"] ); ?></strong></p></div>
			<?php }
			if ( $this->forbid_view ) { ?>
				<div class="error inline bws_visible"><p><strong><?php _e( "Notice", 'simplywordpress' ); ?>:</strong> <strong><?php printf( __( "It is prohibited to view %s settings on this site in the %s network settings.", 'simplywordpress' ), $this->plugins_info["Name"], $this->plugins_info["Name"] ); ?></strong></p></div>
			<?php } else { ?>
				<table class="form-table">			
					...
					if ( ! empty( $this->pro_page ) && $this->bws_hide_pro_option_exist ) { ?>
						<tr>
							<th scope="row"><?php _e( 'Pro Options', 'simplywordpress' ); ?></th>
							<td>
								...
								<span class="bws_info"><?php _e( 'Enable to display plugin Pro options.', 'simplywordpress' ); ?></span>
							</td>
						</tr>
					<?php } ?>
					<tr>
						<th scope="row"><?php _e( 'Track Usage', 'simplywordpress' ); ?></th>
						<td>
							...
							<span class="bws_info"><?php _e( 'Enable to allow tracking plugin usage anonymously in order to make it better.', 'simplywordpress' ); ?></span>
						</td>
					</tr>
					<tr>
						<th scope="row"><?php _e( 'Default Settings', 'simplywordpress' ); ?></th>
						<td>
							<input<?php echo $this->change_permission_attr; ?> name="bws_restore_default" type="submit" class="button" value="<?php _e( 'Restore Settings', 'simplywordpress' ); ?>" />
							<div class="bws_info"><?php _e( 'This will restore plugin settings to defaults.', 'simplywordpress' ); ?></div>
						</td>
					</tr>
				</table>
			<?php }
		...
		public function tab_import_export() { ?>
			<h3 class="bws_tab_label"><?php _e( 'Import / Export', 'simplywordpress' ); ?></h3>
			<?php $this->help_phrase(); ?>
			<hr>						
			<?php /**
			 * action - Display custom options on the Import / Export' tab
		...
		private function save_options_misc() {
			...
			if ( isset( $track_usage ) ) {
				...
				$raw_response = wp_remote_post( 'http://www.simplywordpress.net/', $options );
		...
		public function tab_license() {
			...
			<h3 class="bws_tab_label"><?php _e( 'License Key', 'simplywordpress' ); ?></h3>
			<?php $this->help_phrase(); ?>
			<hr>
			<?php if ( ! empty( $this->pro_page ) ) {
				...
				if ( $this->pro_plugin_is_activated ) { 
					...
					<p><strong><?php _e( 'Congratulations! Pro license is activated successfully.', 'simplywordpress' ); ?></strong></p>
					<p><?php printf( __( 'You will be automatically redirected to the %s in %s seconds.', 'simplywordpress' ), '<a href="' . $this->pro_page . '">' . __( 'Settings page', 'simplywordpress' ) . '</a>', '<span id="bws_timeout_counter">7</span>' ); ?></p>
				<?php } else { 			
					$attr = '';
					...
					<table class="form-table">			
						<tr>
							<th scope="row"><?php _e( 'License Key', 'simplywordpress' ); ?></th>
							<td>
								...
								<input <?php echo $attr; ?> type="submit" class="button button-secondary" name="bws_license_submit" value="<?php _e( 'Activate', 'simplywordpress' ); ?>" />
								<div class="bws_info">
									<?php printf( __( 'Enter your license key to activate %s and get premium plugin features.', 'simplywordpress' ), '<a href="' . $this->plugins_info['PluginURI'] . '?k=' . $this->link_key . '&pn=' . $this->link_pn . '&v=' . $this->plugins_info["Version"] . '&wp_v=' . $wp_version . '" target="_blank" title="' . $this->plugins_info["Name"] . ' Pro">' . $this->plugins_info["Name"] . ' Pro</a>' ); ?>
								</div>
								<?php if ( '' != $attr ) { ?>
									<p><?php _e( "Unfortunately, you have exceeded the number of available tries per day. Please, upload the plugin manually.", 'simplywordpress' ); ?></p>
								<?php }
								if ( $this->trial_days !== false )
									echo '<p>' . __( 'or', 'simplywordpress' ) . ' <a href="' . $this->plugins_info['PluginURI'] . 'trial/?k=' . $this->link_key . '&pn=' . $this->link_pn . '&v=' . $this->plugins_info["Version"] . '&wp_v=' . $wp_version . '" target="_blank">' . sprintf( __( 'Start Your Free %s-Day Trial Now', 'simplywordpress' ), $this->trial_days ) . '</a></p>'; ?>
							</td>
						</tr>
					</table>
				<?php }
				...
				<table class="form-table">			
					<tr>
						...
						<td>
							...
							<input type="submit" class="button button-secondary" name="bws_license_submit" value="<?php _e( 'Check license key', 'simplywordpress' ); ?>" />
							<div class="bws_info">
								<?php _e( 'If necessary, you can check if the license key is correct or reenter it in the field below.', 'simplywordpress' ); ?>
							</div>
						</td>
					</tr>
					<tr>
						<th scope="row"><?php _e( 'Manage License Settings', 'simplywordpress' ); ?></th>
						<td>
							<a class="button button-secondary" href="https://www.simplywordpress.net" target="_blank"><?php _e( 'Login to Client Area', 'simplywordpress' ); ?></a>
							<div class="bws_info">
								<?php _e( 'Manage active licenses, download BWS products, and view your payment history using simplywordpress Client Area.', 'simplywordpress' ); ?>
							</div>
						</td>
					</tr>
				</table>
				...
				if ( strlen( $bws_license_key ) != 18 ) {
					...
					if ( $this->is_pro ) {
						...
						if ( ! empty( $this->all_plugins ) && isset( $current ) && is_array( $current->response ) ) {
							...
							if ( is_wp_error( $raw_response ) || 200 != wp_remote_retrieve_response_code( $raw_response ) ) {
								$error = __( 'Something went wrong. Please try again later. If the error appears again, please contact us', 'simplywordpress' ) . ': <a href=https://www.simplywordpress.net>simplywordpress</a>. ' . __( 'We are sorry for inconvenience.', 'simplywordpress' );
							} else {
								$response = maybe_unserialize( wp_remote_retrieve_body( $raw_response ) );
								if ( is_array( $response ) && !empty( $response ) ) {
									foreach ( $response as $single_response ) {
										if ( "wrong_license_key" == $single_response->package ) {
											...
											$error = __( 'This license key is bound to another site.', 'simplywordpress' );
										} else if ( "time_out" == $single_response->package ) {
											$message = __( 'This license key is valid, but Your license has expired. If you want to update our plugin in future, you should extend the license.', 'simplywordpress' );
										} elseif ( "you_are_banned" == $single_response->package ) {
											$error = __( "Unfortunately, you have exceeded the number of available tries.", 'simplywordpress' );
										} elseif ( "duplicate_domen_for_trial" == $single_response->package ) {
											$error = __( "Unfortunately, the Pro Trial licence was already installed to this domain. The Pro Trial license can be installed only once.", 'simplywordpress' );
										}										
										if ( empty( $error ) ) {
											if ( empty( $message ) ) {
												if ( isset( $single_response->trial ) )
													$message = __( 'The Pro Trial license key is valid.', 'simplywordpress' );
												else
													$message = __( 'The license key is valid.', 'simplywordpress' );

												if ( ! empty( $single_response->time_out ) )
													$message .= ' ' . __( 'Your license will expire on', 'simplywordpress' ) . ' ' . $single_response->time_out . '.';

												if ( isset( $single_response->trial ) && $this->is_trial )
													$message .= ' ' . sprintf( __( 'In order to continue using the plugin it is necessary to buy a %s license.', 'simplywordpress' ), '<a href="' . $this->plugins_info['PluginURI'] . '?k=' . $this->link_key . '&pn=' . $this->link_pn . '&v=' . $this->plugins_info["Version"] . '&wp_v=' . $wp_version . '" target="_blank" title="' . $this->plugins_info["Name"] . '">Pro</a>' );
											}
									...
									if ( is_array( $response ) && ! empty( $response ) ) {
										foreach ( $response as $single_response ) {
											if ( "wrong_license_key" == $single_response->package ) {
												...
												$error = __( "Unfortunately, you have exceeded the number of available tries per day. Please, upload the plugin manually.", 'simplywordpress' );
											} elseif ( "time_out" == $single_response->package ) {
												$error = sprintf( __( "Unfortunately, Your license has expired. To continue getting top-priority support and plugin updates, you should extend it in your %s", 'simplywordpress' ), ' <a href="https://www.simplywordpress.net">Client Area</a>' );
											} elseif ( "duplicate_domen_for_trial" == $single_response->package ) {
										...
										if ( empty( $error ) ) {
											...
											$url = 'http://www.simplywordpress.net/?bws_first_download=' . $bws_license_plugin . '&bws_license_key=' . $bws_license_key . '&download_from=5';
		...
		public function help_phrase() {
			echo '<div class="bws_tab_description">' . __( 'Need Help?', 'simplywordpress' ) . ' ';
			if ( '' != $this->doc_link )
				echo '<a href="' . $this->doc_link . '" target="_blank">' . __( 'Read the Instruction', 'simplywordpress' );
			else
				echo '<a href="https://www.simplywordpress.net/hc/en-us/" target="_blank">' . __( 'Visit Help Center', 'simplywordpress' );
			if ( '' != $this->doc_video_link )
				echo '</a>' . ' ' . __( 'or', 'simplywordpress' ) . ' ' . '<a href="' . $this->doc_video_link . '" target="_blank">' . __( 'Watch the Video', 'simplywordpress' );
			echo '</a></div>';
		}
		...
		public function bws_pro_block_links() { 
			...
			<div class="bws_pro_version_tooltip">							
				<a class="bws_button" href="<?php echo $this->plugins_info['PluginURI']; ?>?k=<?php echo $this->link_key; ?>&amp;pn=<?php echo $this->link_pn; ?>&amp;v=<?php echo $this->plugins_info["Version"]; ?>&amp;wp_v=<?php echo $wp_version; ?>" target="_blank" title="<?php echo $this->plugins_info["Name"]; ?>"><?php _e( 'Upgrade to Pro', 'simplywordpress' ); ?></a>
				<?php if ( $this->trial_days !== false ) { ?>
					<span class="bws_trial_info">
						<?php _e( 'or', 'simplywordpress' ); ?> 
						<a href="<?php echo $this->plugins_info['PluginURI']; ?>?k=<?php echo $this->link_key; ?>&amp;pn=<?php echo $this->link_pn; ?>&amp;v=<?php echo $this->plugins_info["Version"]; ?>&amp;wp_v=<?php echo $wp_version; ?>" target="_blank" title="<?php echo $this->plugins_info["Name"]; ?>"><?php _e( 'Start Your Free Trial', 'simplywordpress' ); ?></a>
					</span>
				<?php } ?>
				<div class="clear"></div>
			</div>
