...
if ( ! class_exists( 'Cptch_Whitelist' ) ) {
	...
	class Cptch_Whitelist extends WP_List_Table {
		...
		*/
		 ...
		 * @return void
		 */
		function cptch_display_whitelist_content() {
			...
			<?php if ( ! ( isset( $_REQUEST['cptch_show_whitelist_form'] ) || isset( $_REQUEST['cptch_add_to_whitelist'] ) ) ) { ?>
				<form method="post" action="admin.php?page=cptc_dashboard&amp;action=simply_secure&amp;stab=whitelist"  style="margin: 10px 23px;">
					<table>
						<tr>
							<td>
								<label>
						...
						<tr>
							<td class="cptch_whitelist_buttons">
								...
								<div class="alignleft">
									...
									<?php if ( ! $hidden ) {
										$content = sprintf( __( 'By click on this button, all IP-addresses from the whitelist of %s will be loaded to the whitelist of %s', 'captcha' ), $limit_attempts_info['name'], 'Captcha by simplywordpress' );
			...
			<?php } ?>
			<form class="form-table cptch_whitelist_form" method="post" action="admin.php?page=cptc_dashboard&amp;action=simply_secure&amp;stab=whitelist" style="margin: 10px 24px;<?php echo ! ( isset( $_REQUEST['cptch_show_whitelist_form'] ) || isset( $_REQUEST['cptch_add_to_whitelist'] ) ) ? 'display: none;': ''; ?>">
			...
			<?php if ( ! $hidden ) { ?>
				<form id="cptch_whitelist_search" method="post" action="admin.php?page=cptc_dashboard&amp;action=simply_secure&amp;stab=whitelist">
					<?php $this->search_box( __( 'Search IP', 'captcha' ), 'search_whitelisted_ip' );
					wp_nonce_field( $this->basename, 'cptch_nonce_name' ); ?>
				</form>
				<form id="cptch_whitelist" method="post" action="admin.php?page=cptc_dashboard&amp;action=simply_secure&amp;stab=whitelist">
					<?php $this->display();
					wp_nonce_field( $this->basename, 'cptch_nonce_name' ); ?>
				</form>
			<?php }
		...
		*/
		...
		function column_ip( $item ) {
			...
			$url      = "?page=cptc_dashboard&amp;action=simply_secure&amp;stab=whitelist&cptch_remove={$item['id']}{$order_by}{$order}{$paged}{$s}";
			$actions = array(
