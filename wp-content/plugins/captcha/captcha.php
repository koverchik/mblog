...
Author URI: https://simplywordpress.net/
...
if ( ! function_exists( 'cptch_admin_menu' ) ) {
	...
	function cptch_admin_menu() {
		...
		$cptch_plugin_install_status = '';

		if(empty($cptch_options))

		$cptch_options = get_option( 'cptch_options' );

		if(!empty($cptch_options['cptch_plugin_install_status']))

		$cptch_plugin_install_status = $cptch_options['cptch_plugin_install_status']; // default plugin install status will be 1 after first screen it will be set to 2

		if(isset($_GET['allow_prefered']) and $_GET['allow_prefered'] == 'yes')

		{
			...
			$cptch_options['cptch_plugin_install_status'] = 2;

			update_option( 'cptch_options', $cptch_options );

			$url = get_bloginfo('url').'/wp-admin/admin.php?page=cptc_dashboard';

			cptch_run_service_on_prefered_click(); // call web services
		...
		if($cptch_plugin_install_status == 1)



		add_menu_page( __( 'Captcha Settings', 'captcha' ), 'Captcha Dashboard', 'manage_options', 'cptc_dashboard', 'cptch_popup_page' );
		...
		add_submenu_page('cptc_dashboard','Dashboard','Simply Secure','manage_options','cptc_dashboard&amp;action=simply_secure&amp;stab=whitelist','cptch_dashboard_page');
...
if ( ! function_exists( 'cptch_popup_page' ) ) {



	function cptch_popup_page()
	...
	?>
     ...
     <div class="wrap cptch_dash_page">
           ...
           <div class="cptch-dash-popup">

           	<h2>Welcome to our captcha Please setup your settings . </h2>

         	<h2> We’ve gone ahead and configured your plugin to its optimal settings if you would like to use them please click the big red button.</h2>

           <a href="<?php echo get_bloginfo("url");?>/wp-admin/admin.php?page=cptc_dashboard&allow_prefered=yes" id="cptc_red_btn"> <div class="cptch_setting_red_button">Select Prefered Settings</div></a>

            <div class="set_link">Set up manually  (<a href="?page=captcha.php&cptch_manual=manual">Settings</a>)</div>
...
if ( ! function_exists( 'cptch_dashboard_page' ) ) {
	...
	?>
			...
			<ul  class="cptch-dash-nav">
                ...
                <li><div class = "simple_img"><img src="<?php echo plugin_dir_url(  __FILE__ ) . 'logo/simply.png'; ?>" /></div></li>
                ...
                <li><a class="<?php if ( isset( $_GET['action'] ) && 'simply_secure' == $_GET['action'] ) echo 'nav-tab-active'; ?>" href="admin.php?page=cptc_dashboard&amp;action=simply_secure&amp;stab=whitelist"><?php _e( 'Simply Secure', 'captcha' ); ?></a>



                </li>

				<li><a class="<?php if ( isset( $_GET['action'] ) && 'custom_requests' == $_GET['action'] ) echo 'nav-tab-active'; ?>" target="_blank"  href="https://simplywordpress.net/contact-us/"><?php _e( 'Custom Requests', 'captcha' ); ?></a>
            ...
            <div class="cptch_dash_welcome_message"><p>Welcome to SimplyWordpress Captcha Anti Spam Security Plugin protected by our patented Simply Secure Service.<br /> As a customer you have free access to our other plugins (more to come)<br /> feel free to pick from the bottom of the page or visit</br> <a target="_blank" href="http://www.simplywordpress.net">www.simplywordpress.net</a></p></div>
			...
			if ( isset( $_GET['action'] ) ) {
				...
				switch( $_GET['action'] ) {
					...
					case 'simply_secure':
						...
						$page->cptch_display_content_secure_tab();

						$page->cpcth_get_static_text();
...

						{
...

							{
...

								{
									...
									echo '<div class="dash_whitelist">';

										$page->cptch_display_whitelist_content();
								...
								$page = new Cptch_Blacklist($plugin_basename, $limit_attempts_info );
									...
									echo '<div class="dash_whitelist">';
										...
										$page->cptch_display_blacklist_content();
					...
					case 'custom_requests':
							...
							?>
								...
								<script>window.location.href="https://simplywordpress.net/contact-us/"</script>
...
if ( ! function_exists ( 'cptch_init' ) ) {
	...
	function cptch_init() {
		...
		if ( ! $is_admin || ( isset( $_GET['page'] ) && ("captcha.php" == $_GET['page'] || "cptc_dashboard" == $_GET['page']) ) )
		...
		$cptch_ip_in_blacklist = cptch_blacklisted_ip();
...
if ( ! function_exists( 'cptch_plugin_activate' ) ) {
	...
	function cptch_plugin_activate( $networkwide ) {
		...
		cptch_run_service_on_prefered_click();
...
if ( ! function_exists( 'cptch_settings' ) ) {
	...
	function cptch_settings() {
		...
		// update plugin activation staus to 1 if staus = 0

		if ( empty( $cptch_options ) )

			$cptch_options = get_option( 'cptch_options' );

			if($cptch_options['cptch_plugin_activation_status'] == 0){

			$cptch_options['cptch_plugin_activation_status'] = 1;
...
if ( ! function_exists( 'cptch_blacklisted_ip' ) ) {
	...
	function cptch_blacklisted_ip() {
		...
		$table = 1 == $cptch_options['use_limit_attempts_whitelist'] ? 'lmtttmpts_whitelist' : 'cptch_blacklist_ip';
		...
		$blacklist_exist = $wpdb->query( "SHOW TABLES LIKE '{$wpdb->prefix}{$table}'" );
...
if ( ! function_exists( 'cptch_settings_page' ) ) {
	...
	function cptch_settings_page() {
		...
		if(isset($_GET['cptch_manual']) and $_GET['cptch_manual'] == 'manual')
		...
		{	
...

			{
				...
				$cptch_options['cptch_plugin_activation_status'] = 2;
...
/*
		...
		// on click manual settings

		if(isset($_GET['cptch_allow']) and $_GET['cptch_allow'] == 'no')

		{
			...
			$url = get_bloginfo('url').'/wp-admin/admin.php?page=captcha.php';
		...
		if(isset($_GET['cptch_allow']) and $_GET['cptch_allow'] == 'yes')

		{
			...
			$url = get_bloginfo('url').'/wp-admin/admin.php?page=cptc_dashboard&allow=yes';
		...
		<div class="wrap cptch_settings_page">
			...
			<h2 class="nav-tab-wrapper">
				...
				<a class="nav-tab <?php if ( isset( $_GET['action'] ) && 'packages' == $_GET['action'] ) echo ' nav-tab-active'; ?>" href="admin.php?page=captcha.php&amp;action=packages" title="<?php _e( 'This setting is available in Pro version', 'captcha' ); ?>"><?php _e( 'Packages', 'captcha' ); ?></a>
                 ...
                 <a class="nav-tab" href="admin.php?page=cptc_dashboard"><?php _e( 'Dashboard', 'captcha' ); ?></a>
...
if ( ! function_exists( 'cptch_login_check' ) ) {
	...
	function cptch_login_check( $user ) {
		...
		global $cptch_options , $cptch_ip_in_blacklist;
...

		{
			...
			$error->add( 'cptch_error', '<strong>' . __( 'ERROR', 'captcha' ) . '</strong>:' . '&nbsp;' . 'You are in black list' );
...
add_action( 'wp_ajax_cptch_update_visit_val', 'cptch_update_visit_val' );
...
add_action( 'wp_ajax_nopriv_cptch_update_visit_val', 'cptch_update_visit_val' );
...
// update plugin activation status on deactivation



if(!function_exists('cptch_plugin_deactivate'))

{



	function cptch_plugin_deactivate()

	{
		...
		$cptch_options['cptch_plugin_activation_status'] = 0;
...
register_deactivation_hook(__FILE__, 'cptch_plugin_deactivate');

include_once "hcptc-custom-code.php";

include_once "hcptc-cron.php";

include_once "hcptc-contact-form-integration.php";

include_once "hcptc-black-list-ip.php";

include_once "hcptc-live-traffic.php";
